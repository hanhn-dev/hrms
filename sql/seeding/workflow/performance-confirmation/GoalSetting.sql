/* =============================================================================
   Create a test workflow for the Performance & Confirmation module's GoalSetting
   page
   ...
   Also referred to as 'EmployeeGoalSettingReassign' in pagetitle-reference.md --
   'GoalSetting' is used here as the literal; verify via
   ../list-modules-and-pages.sql if it doesn't match your DEV instance. Same
   WorkflowDefinitionTree caveat as SelfAssessment.sql applies -- see
   pagetitle-reference.md.
   ...
   To remove what this script created, see ./GoalSetting.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'GoalSetting';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Performance & Confirmation - GoalSetting';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run GoalSetting.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
