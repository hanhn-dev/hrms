/* =============================================================================
   Create a test workflow for the Performance Assessment module's SalaryRecommendations page
   ...
   Salary recommendations approval. Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./SalaryRecommendations.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'SalaryRecommendations';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Performance Assessment - SalaryRecommendations';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run SalaryRecommendations.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
