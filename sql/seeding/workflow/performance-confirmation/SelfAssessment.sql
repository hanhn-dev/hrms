/* =============================================================================
   Create a test workflow for the Performance & Confirmation module's
   SelfAssessment page
   ...
   CAUTION per pagetitle-reference.md: this page's WorkflowDefinitionTree is read
   directly at approval time (confirmed in USP_WorkFlow_Routing_Levels.sql), not
   via TWorkflowDetails -- the rows this script creates won't produce a working
   approval chain here; build a real chain through the Admin > Workflow
   Management UI instead. This script still guards against the exact reserved
   WorkflowName 'Self assessment' to avoid tripping that branch.
   ...
   To remove what this script created, see ./SelfAssessment.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'SelfAssessment';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Performance & Confirmation - SelfAssessment';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run SelfAssessment.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
