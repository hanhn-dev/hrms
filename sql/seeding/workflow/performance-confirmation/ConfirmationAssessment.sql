/* =============================================================================
   ...
   Same WorkflowDefinitionTree caveat as SelfAssessment.sql applies -- read
   directly at approval time, not via TWorkflowDetails; build a real chain
   through the Admin > Workflow Management UI instead.
   ...
   To remove what this script created, see ./ConfirmationAssessment.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'ConfirmationAssessment';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Performance & Confirmation - ConfirmationAssessment';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run ConfirmationAssessment.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
