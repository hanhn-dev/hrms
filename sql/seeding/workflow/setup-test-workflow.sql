/* =============================================================================
   Create a test approval workflow (TWorkflowManagement + N TWorkflowDetails
   level rows) for an EXISTING employer in DEV, so you can exercise
   SP_ApproveWorkFlowRequest / SP_RejectWorkFlowRequest end to end.
   ...
   RUN list-modules-and-pages.sql FIRST -- it shows your target employer's
   actual live modules, workflow-eligible pages, existing roles, and any
   workflows already configured (in the same 'RoleName[RoleID]' format the
   Define Workflow screen itself shows, e.g. 'HR[6]'). Modules/pages are
   per-employer runtime config with no fixed master list in source, so that
   script -- not this comment -- is the authoritative way to pick values.
   pagetitle-reference.md has additional business-feature context for pages
   I could independently verify from source.
   ...
   request-creation callers use to resolve approvers) -- READ BEFORE RUNNING:
     ...
     - @ModulePageName must exactly match the literal @PageTitle string the
       ...
       list-modules-and-pages.sql (live) or pagetitle-reference.md (source-verified
       examples) BEFORE running this; an unverified guess silently creates a
       workflow definition nothing ever looks up.

   To remove what this script created, see cleanup-test-workflow.sql.
   ========================================================================== */
...
DECLARE @ModulePageName   VARCHAR(200)  = 'LEAVEREQUEST';  -- <<< run list-modules-and-pages.sql first
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave Approval';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run cleanup-test-workflow.sql with @WorkflowIdToDelete = ' + CAST(@WorkflowId AS VARCHAR(10))
