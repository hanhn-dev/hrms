/* =============================================================================
   Create a test workflow for the Policy Documents module's SelfAttestation page
   ...
   Policy self attestation approval. Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./SelfAttestation.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'SelfAttestation';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Policy Documents - SelfAttestation';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run SelfAttestation.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
