/* =============================================================================
   Create a test workflow for the Policy Documents module's PolicyDocumentActivation page
   ...
   Policy document activation approval. Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./PolicyDocumentActivation.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'PolicyDocumentActivation';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Policy Documents - PolicyDocumentActivation';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run PolicyDocumentActivation.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
