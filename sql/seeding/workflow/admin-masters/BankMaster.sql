/* =============================================================================
   Create a test workflow for the Admin Masters module's BankMaster page
   ...
   Typo'd as 'BankMAster' in a couple of procs (same row, case-insensitive
   collation) -- 'BankMaster' is used here as the canonical literal; verify via
   ../list-modules-and-pages.sql if it doesn't match your DEV instance.
   ...
   To remove what this script created, see ./BankMaster.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'BankMaster';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Admin Masters - BankMaster';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run BankMaster.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
