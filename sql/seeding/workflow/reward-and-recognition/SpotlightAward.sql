/* =============================================================================
   Create a test workflow for the Reward & Recognition module's SpotlightAward page
   ...
   Spotlight award approval. Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./SpotlightAward.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'SpotlightAward';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Reward & Recognition - SpotlightAward';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run SpotlightAward.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
