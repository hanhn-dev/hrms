/* =============================================================================
   Create a test workflow for the Reward & Recognition module's JuryAward page
   ...
   Jury award template approval. Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./JuryAward.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'JuryAward';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Reward & Recognition - JuryAward';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run JuryAward.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
