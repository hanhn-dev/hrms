/* =============================================================================
   Create a test workflow for the My Details module's EndOfContractUpdation page
   ...
   Confirmed via HRMS-DATABASE/HRMS/DML/74592/Insert TModulePages.sql:
   PageModuleId = 1 (My Details), IsWorkflowAvailable = 1, MappedAppPage =
   MyDetails.aspx. Update notification when an employee's contract end date has been changed.
   ...
   To remove what this script created, see ./EndOfContractUpdation.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'EndOfContractUpdation';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - My Details - EndOfContractUpdation';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run EndOfContractUpdation.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
