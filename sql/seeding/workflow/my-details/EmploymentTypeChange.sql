/* =============================================================================
   Create a test workflow for the My Details module's EmploymentTypeChange page
   ...
   This same page title is shared by every My Details sub-form (contact, bank,
   family, education, nomination, visa, passport, custom fields, profile picture,
   etc., ~20 sub-forms) -- confirmed module = My Details.
   ...
   To remove what this script created, see ./EmploymentTypeChange.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
------------------------------------------------------------------------------
DECLARE @TargetEmployerId INT           = 10;              -- <<< existing Employerid in DEV to add this workflow to
DECLARE @ModulePageName   VARCHAR(200)  = 'EmploymentTypeChange';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - My Details - EmploymentTypeChange';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run EmploymentTypeChange.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
