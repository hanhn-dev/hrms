/* =============================================================================
   Create a test workflow for the Recruitment module's IJP Approval page
   ...
   Internal job posting (IJP) approval. Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./IJPApproval.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'IJP Approval';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Recruitment - IJP Approval';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run IJPApproval.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
