/* =============================================================================
   Create a test workflow for the Recruitment module's RecruitmentManagement page
   ...
   Recruitment/job requisition page. pagetitle-reference.md flags Recruitment as
   also supporting role-based approvers ('B'/'D'/'H'/'M' via
   TRoles/TRecruiter/TEmployeeOrgBusinessHead) as an alternative to this script's
   'U'/'R'/'F' engine -- ask if you need that variant set up.
   ...
   To remove what this script created, see ./RecruitmentManagement.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'RecruitmentManagement';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Recruitment - RecruitmentManagement';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run RecruitmentManagement.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
