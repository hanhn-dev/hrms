/* =============================================================================
   Create a test workflow for the Recruitment module's RA-InitiateReallocation
   page
   ...
   Recruiter allocation page; see pagetitle-reference.md for Recruitment's
   alternate role-based approver codes.
   ...
   To remove what this script created, see ./RA-InitiateReallocation.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'RA-InitiateReallocation';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Recruitment - RA-InitiateReallocation';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run RA-InitiateReallocation.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
