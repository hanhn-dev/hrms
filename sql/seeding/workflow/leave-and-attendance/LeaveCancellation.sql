/* =============================================================================
   Create a test workflow for the Leave & Attendance module's LeaveCancellation
   page
   ...
   This is the page used when the employer's IsLeavePullBackWorkFlowEnabled flag
   is ON; when OFF, leave cancellation instead routes through the
   'LeaveRequest'/'LEAVEREQUEST' workflow -- see pagetitle-reference.md.
   ...
   To remove what this script created, see ./LeaveCancellation.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'LeaveCancellation';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave & Attendance - LeaveCancellation';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run LeaveCancellation.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
