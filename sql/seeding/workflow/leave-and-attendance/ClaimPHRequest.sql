/* =============================================================================
   Create a test workflow for the Leave & Attendance module's ClaimPHRequest page
   ...
   Older code also uses 'ClaimPH' for this same feature (same underlying
   TModulePages row) -- 'ClaimPHRequest' is used here as the canonical value;
   verify via ../list-modules-and-pages.sql if it doesn't match your DEV
   instance.
   ...
   To remove what this script created, see ./ClaimPHRequest.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'ClaimPHRequest';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave & Attendance - ClaimPHRequest';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run ClaimPHRequest.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
