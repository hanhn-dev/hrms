/* =============================================================================
   Create a test workflow for the Leave & Attendance module's OptionalHolidayRequest page
   ...
   Reuses the app's own creation procs -- SP_AdminWM_AddDefinition /
   SP_AdminWM_AddDefinitionDet -- the same procs the Admin > Workflow
   Management screen calls.

   Optional holiday request using the live mixed-case ModulePageName. Distinct from OPTIONALHOLIDAYREQUEST.sql because Windows FS is case-insensitive.

   See ../list-modules-and-pages.sql, ../pagetitle-reference.md, and
   ../cleanup-test-workflow.sql.
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'OptionalHolidayRequest';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave & Attendance - OptionalHolidayRequest';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run OptionalHolidayRequest-Live.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
