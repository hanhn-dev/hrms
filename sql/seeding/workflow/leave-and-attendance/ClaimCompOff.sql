/* =============================================================================
   Create a test workflow for the Leave & Attendance module's ClaimCompOff page
   ...
   Renamed from the older 'CompOffRequest' literal -- 'ClaimCompOff' is used here
   as the canonical value; verify which name is live in your DEV instance via
   ../list-modules-and-pages.sql.
   ...
   To remove what this script created, see ./ClaimCompOff.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'ClaimCompOff';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave & Attendance - ClaimCompOff';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run ClaimCompOff.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
