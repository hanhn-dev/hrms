/* =============================================================================
   Create a test workflow for the Leave & Attendance module's WorkFromHome page
   ...
   Gets remapped internally to 'AttendanceRegularize-WFH' before the TModulePages
   lookup -- this script deliberately still passes 'WorkFromHome', not the
   remapped name, per pagetitle-reference.md.
   ...
   To remove what this script created, see ./WorkFromHome.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'WorkFromHome';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave & Attendance - WorkFromHome';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run WorkFromHome.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
