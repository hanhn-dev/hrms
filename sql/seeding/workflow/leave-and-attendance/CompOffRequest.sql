/* =============================================================================
   Create a test workflow for the Leave & Attendance module's CompOffRequest page
   ...
   Comp-off request (live name; ClaimCompOff may also exist after rename). Confirmed live for employer 10 via list-modules-and-pages.sql /
   ...
   To remove what this script created, see ./CompOffRequest.cleanup.sql (or ../cleanup-test-workflow.sql for WorkflowId / bulk).
   ========================================================================== */
SET NOCOUNT ON;
...
DECLARE @ModulePageName   VARCHAR(200)  = 'CompOffRequest';  -- FIXED for this script -- do not change
DECLARE @WorkflowName     VARCHAR(100)  = 'TEST - Leave & Attendance - CompOffRequest';
...
IF @NewRoleName IS NOT NULL
    PRINT 'To remove this workflow, run CompOffRequest.cleanup.sql with @TargetEmployerId = ' + CAST(@TargetEmployerId AS VARCHAR(10))
