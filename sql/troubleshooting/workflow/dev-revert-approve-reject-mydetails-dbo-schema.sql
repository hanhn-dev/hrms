-- =============================================================================
-- DEV-ONLY revert: restore Sp_ApproveRejectMyDetailsReview as it was before
-- the dbo-schema filter patch (Clone.TEmployeeFamilyDetails / BankDetails).
--
-- Run this on DEV after testing to put the procedure back.
-- Do not run on QA / UAT / PROD.
-- =============================================================================
...
AND TR.ChangeRequestId = @LV_ChangeRequestId                    
  ...
  Occupation,Gender,OtherInsurance,Dependant,GraduationDate,Address,Comments,Minor,SSN,                
  GuardianAddress,GuardianName,Smoker,IsSubmit,IsDelete,CreatedBy,CreatedDate,UpdatedBy,                
  UpdatedDate,AadharNumber, CreatedDateUtc,UpdatedDateUtc,getdate() from TEmployeeFamilyDetails with(nolock)                
  WHERE EmployeeId = @EmployeeID AND                 
  EmployeeFamilyDetailID = (Select top 1 ChildRowId from TMyDetailsChangeRequestDetails where ChangeRequestId = @LV_ChangeRequestId)                
...
BEGIN                    
  ...
  ,NomineeEmailID,NomineeQualification,NomineeRemarks                
  ,IsMinor,NomineeGuardianName,NomineeGuardianAddress,NomineeGuardianRelwithNominee                
  ,NomineeGuardianContactNumber, getdate(),CreatedDateUtc,UpdatedDateUtc           from TEmployeeNomination WITH(NOLOCK)                
  WHERE EmployeeId = @EmployeeID AND (EmployeeNominationId = (Select top 1 ChildRowId from TMyDetailsChangeRequestDetails where ChangeRequestId = @LV_ChangeRequestId)                
  OR EmployeeFamilyDetailID = (Select top 1 ChildRowId from TMyDetailsChangeRequestDetails where ChangeRequestId = @LV_ChangeRequestId))                
