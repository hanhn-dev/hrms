...
Given a module/feature name supplied with this command (the text after
`/document-feature`; also `$ARGUMENTS` when the host expands it), explore
both repos, and write a single developer-facing guide connecting the
application code to the database, with a workflow diagram, a request-journey
diagram (where a request starts and where it ends), and a
...
1. **Resolve the feature name.** Take it from the invocation argument
   ...
   after `/document-feature`). If empty, stop and ask for one.
...
5. **Write the guide** to
   ...
   ## Reference
   <Confidence note plus the files/pages this guide was built from, grouped
   under ### SourceCode and ### TDG HRMS DB (llm-wiki citations go under the
   DB group). The analyzed date already appears next to the title — don't
   repeat it here unless you need it in the confidence sentence. Do **not**
   put the source list in YAML frontmatter.>
   ````
   ...
   points/call-chain/procedure reference tables, *then* Known gaps, *then*
   Reference last. Don't front-load accuracy-correction callouts or the
   source list before the Overview — anchor each callout to the specific
   section it corrects, and keep the source inventory at the end so it
   doesn't crowd the narrative.
