...
application code to the database, ending in a workflow diagram and a
...
5. **Write the guide** to
   ...
   *then* the entry points/call-chain/procedure reference tables, *then*
   Known gaps, *then* Reference last. Don't front-load accuracy-correction
   callouts or the source list before the Overview — anchor each callout to
   the specific section it corrects, and keep the source inventory at the
   end so it doesn't crowd the narrative.
