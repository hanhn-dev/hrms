...
|---|---|---:|---:|---:|---:|---|
| `HRMS` | `HRMS_PROD` | 1110 | 4261 | 307 | 155 | Core HR: employees, employers (tenants), roles/access, leave, attendance, payroll/salary, resignation, BGV, recruitment (RRS), confirmation (CMS), performance (PMS), the approval engine, ELMAH error log. |
...
| `HRMS_TRAVELNEXPENSE` | `TravelNExpense_Prod` | 59 | 279 | 22 | 4 | Travel requests, itineraries, multi-category expense claims (`TEXPENSE_*`), advances, payment setup. |
...
- **License insights** — `TLicenseInsight` + `TLicenseInsightSetting` +
  `TLicenseInsightNotification`; `USP_LicenseInsight_*` / `SP_Employer_LicenseInsight_*`.
