...
describe('db_execute_stored_procedure', () => {
  ...
  it('returns the SQLite unsupported stored-procedure execute response through the MCP tool surface', async () => {
    ...
    try {
      ...
      expect(parsed.operation).toBe('db_execute_stored_procedure');
      expect(parsed.error).toContain('not implemented for sqlite');
    } finally {
      await session.close();
    }
  });
